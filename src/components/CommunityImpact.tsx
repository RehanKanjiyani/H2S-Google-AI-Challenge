...
import { sortAndRankLeaderboard } from '../utils/gamification';
...
export default function CommunityImpact({ logs, stats, userName, onChangeUserName }: CommunityImpactProps) {
  ...
  const sortedLeaderboard = sortAndRankLeaderboard(rawLeaderboard);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="community-root">
      ...
      <div className="space-y-6 lg:col-span-2">
        <section aria-labelledby="alliance-heading" className="bg-white rounded-2xl p-6 border border-slate-100 shadow-xs">
          <div className="flex items-center space-x-2 pb-4 border-b border-slate-100">
            <Globe className="w-5 h-5 text-emerald-600" aria-hidden="true" />
            <h2 id="alliance-heading" className="text-base font-semibold text-slate-800 font-display">Global EcoTrack Alliance</h2>
          </div>
          ...
          <div className="bg-[#15803d]/5 border border-emerald-100 rounded-xl p-4 mt-4 flex items-start space-x-3">
            <Sparkles className="w-5 h-5 text-[#15803d] shrink-0 mt-0.5" aria-hidden="true" />
            <div>
        ...
        <section aria-labelledby="nickname-heading" className="bg-white rounded-2xl p-6 border border-slate-100 shadow-xs">
          <h3 id="nickname-heading" className="text-sm font-semibold text-slate-800 mb-1 font-display">Manage Leaderboard Nickname</h3>
          ...
          <form onSubmit={handleNameSubmit} className="flex gap-2 max-w-md">
            <label htmlFor="nickname-input" className="sr-only">Leaderboard Nickname</label>
            <input 
              type="text" 
              id="nickname-input"
              className="bg-slate-50 border border-slate-100 rounded-lg p-2.5 text-xs text-slate-700 w-full focus:outline-hidden focus-visible:ring-2 focus-visible:ring-emerald-500"
              value={typedName}
            ...
            <button
              type="submit"
              className="bg-[#15803d] hover:bg-emerald-800 text-white px-4 py-2 text-xs rounded-lg font-semibold flex items-center space-x-1.5 transition shrink-0 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-emerald-500"
            >
          ...
          {successMsg && (
            <p className="text-emerald-700 text-[11px] mt-2 font-semibold" aria-live="polite">
      ...
      {/* Leaderboard Section */}
      <aside className="bg-white rounded-2xl p-6 border border-slate-100 shadow-xs" id="leaderboard-sidebar" aria-labelledby="leaderboard-heading">
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-4">
          <div className="flex items-center space-x-2">
            <Trophy className="w-4 h-4 text-amber-500" aria-hidden="true" />
            <h3 id="leaderboard-heading" className="text-sm font-semibold text-slate-800 font-display">Sustainability Leaders</h3>
          </div>
          <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full font-bold">Week 24</span>
        </div>

        <div className="space-y-2 max-h-[360px] overflow-y-auto pr-1" role="list" aria-label="Community ranking leaderboard">
          {sortedLeaderboard.map((entry) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-right" aria-label={`${entry.points} experience points`}>
                  <span className="text-xs font-bold font-display block text-slate-800">{entry.points}</span>
                  <span className="text-[8px] text-slate-400 block uppercase font-bold" aria-hidden="true">XP</span>
                </div>
              </div>
            );
          })}
